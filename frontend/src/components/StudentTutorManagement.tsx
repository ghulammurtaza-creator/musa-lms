...
export default function StudentTutorManagement() {
  ...
  const fetchAllUsers = useCallback(async () => {
    try {
      const [studentsRes, tutorsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/auth/students`, {
          headers: { 'Authorization': `Bearer ${token}` },
        }),
        fetch(`${API_BASE_URL}/auth/tutors`, {
          headers: { 'Authorization': `Bearer ${token}` },
        }),
      ]);
  ...
  const fetchRelationships = async (studentId: number) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/relationships/student/${studentId}/tutors`,
        {
          headers: { 'Authorization': `Bearer ${token}` },
        }
      );
  ...
  const handleAssign = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/relationships/assign`, {
        method: 'POST',
        headers: {
  ...
  const handleUnassign = async (studentId: number, tutorId: number) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/relationships/unassign`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
